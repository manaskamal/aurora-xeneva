...
static void pointslope(privpath_t *pp, int i, int j, dpoint_t *ctr, dpoint_t *dir) {
  ...
  lambda2 = (a+c+(double)sqrtf((a-c)*(a-c)+4*b*b))/2; /* larger e.value */
  ...
  if (fabs(a) >= fabs(c)) {
    l = (double)sqrtf(a*a+b*b);
    if (l!=0) {
      dir->x = -b/l;
      dir->y = a/l;
    }
  } else {
    l = (double)sqrtf(c*c+b*b);
    if (l!=0) {
      dir->x = -c/l;
      dir->y = b/l;
    }
...
static double ddist(dpoint_t p, dpoint_t q) {
  return (double)sqrtf(sq(p.x-q.x)+sq(p.y-q.y));
}
...
static int opti_penalty(privpath_t *pp, int i, int j, opti_t *res, double opttolerance, int *convc, double *areac) {
  ...
  alpha = 2 - (double)sqrtf(4 - R / 0.3);  /* overall alpha for p0-o-p3 curve */
