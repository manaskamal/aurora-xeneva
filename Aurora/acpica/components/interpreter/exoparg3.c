    ...
    ACPI_WALK_STATE         *WalkState)
{
    ...
    case AML_FATAL_OP:          /* Fatal (FatalType  FatalCode  FatalArg) */
        ...
        Fatal = (ACPI_SIGNAL_FATAL_INFO*)ACPI_ALLOCATE (sizeof (ACPI_SIGNAL_FATAL_INFO));
        if (Fatal)
        {
    ...
    ACPI_WALK_STATE         *WalkState)
{
    ...
    case AML_MID_OP:    /* Mid (Source[0], Index[1], Length[2], Result[3]) */
        ...
        case ACPI_TYPE_STRING:
            ...
            Buffer = (char*)ACPI_ALLOCATE_ZEROED ((ACPI_SIZE) Length + 1);
            if (!Buffer)
            {
                Status = AE_NO_MEMORY;
                goto Cleanup;
        ...
        case ACPI_TYPE_BUFFER:
            ...
            if (Length > 0)
            {
                ...
                Buffer = (char*)ACPI_ALLOCATE_ZEROED (Length);
                if (!Buffer)
                {
                    Status = AE_NO_MEMORY;
                    goto Cleanup;
