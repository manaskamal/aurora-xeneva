    ...
    void                    *Context)
{
    ...
    if (!ObjDesc)
    {
        ...
        Status = AcpiNsAttachObject ((ACPI_NAMESPACE_NODE*)Device, ObjDesc, Node->Type);
    ...
    void                    *Context)
{
    ...
    NewSciHandler = (ACPI_SCI_HANDLER_INFO*)ACPI_ALLOCATE (sizeof (ACPI_SCI_HANDLER_INFO));
    if (!NewSciHandler)
    {
    ...
    void                    *Context)
{
    ...
    Handler = (ACPI_GPE_HANDLER_INFO*) ACPI_ALLOCATE_ZEROED (sizeof (ACPI_GPE_HANDLER_INFO));
    if (!Handler)
    {
        Status = AE_NO_MEMORY;
        goto UnlockAndExit;
