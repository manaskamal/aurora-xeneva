...
static uint32_t codec_query (int codec, int nid, uint32_t payload) {
	...
	uint32_t verb = ((codec & 0xf) << 28) | 
		...
		(payload & 0xfffff);
	//uint32_t verb = 0xF00;
//	printf ("Requesting VERB -> %x\n", verb);
	corb_write(verb);
	//while ((_aud_inl_(RIRBSTS) & 1) == 0)
	//	;
	rirb_read(&response);
...
void hda_reset() {
	...
	//wakeen_bit |= 1 << 1;
	_aud_outw_(WAKEEN,0xffff);    //0xffff
	...
	int_ctl |= (1 << 1);
	int_ctl |= (1 << 2);
	_aud_outl_(INTCTL, 0x800000ff);   //0x800000ff
...
void hda_initialize () {
	...
	if (!pci_find_device_id (0x2668, 0x8086, &pci_dev)){
