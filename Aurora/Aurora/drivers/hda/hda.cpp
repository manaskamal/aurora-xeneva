...
unsigned char* default_data = 0;
unsigned char* play_pos = NULL;
...
void hda_init_output () {
	uint16_t format = (0<<14) | (0<<11) | (0<<8)| (1<<4) | (1<<0);  //0<<15 | 0<<14 | 1<<4 | 1;
...
int hda_get_buffer_number () {
	return _ihd_audio.buffer_completed;
}
