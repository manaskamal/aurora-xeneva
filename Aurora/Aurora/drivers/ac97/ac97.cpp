...
void ac97_initialize () {
	...
	int bus, dev, func = 0;
	if (!pci_find_device_class (0x04,0x01,addr, &bus, &dev, &func)) {
	...
	x64_outportw (_ac97.nambar + PORT_NAM_RESET, 0 /* 42*/);
	...
	x64_outportb(_ac97.nabmbar + NABM_OFFSET_BUFFER_CNT, (1<<0) | (1<<1));
	x64_outportb(_ac97.nabmbar + NABM_OFFSET_BUFFER_CNT, (1<<4));
