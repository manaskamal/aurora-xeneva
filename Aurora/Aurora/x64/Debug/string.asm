...
; 24   : 
; 25   : 	size_t count;
; 26   : 	for (count = 0; count < length; count++)
...
; 27   : 	{
; 28   : 		if (((unsigned char*)first)[count] != ((unsigned char *) second)[count])
...
; 29   : 		{
; 30   : 			if (((unsigned char*)first)[count] < ((unsigned char *)second)[count])
...
; 40   : 
; 41   : 	return (0); //return successful code
...
; 16   : void memset(void *targ, uint8_t val, uint32_t len){
...
; 112  : char* strchr (char* str, int character) {
...
; 85   : 	unsigned count;
; 86   : 
; 87   : 	if ((destString == (char*) NULL) || (sourceString == (char*) NULL))
...
; 90   : 	}
; 91   : 
; 92   : 	if (maxLength > MAX_STRING_LENGTH)

	cmp	QWORD PTR maxLength$[rsp], 25
	jbe	SHORT $LN6@strncpy

; 93   : 		maxLength = MAX_STRING_LENGTH;

	mov	QWORD PTR maxLength$[rsp], 25
$LN6@strncpy:

; 94   : 
; 95   : 	for (count = 0; count < maxLength; count++ )
...
; 96   : 	{
; 97   : 		destString[count] = sourceString[count];
...
; 102  : 
; 103  : 	if (count >= MAX_STRING_LENGTH)

	cmp	DWORD PTR count$[rsp], 25
	jb	SHORT $LN1@strncpy

; 104  : 	{
; 105  : 		return (destString = NULL);
...
; 79   : 
; 80   : 	return (n==0)? 0: (size_t)((unsigned char)*s1 - (unsigned char)*s2);
...
; 48   : 	while (!(res = *(unsigned char*)str1 - *(unsigned char*)str2) && *str2)
