; Listing generated by Microsoft (R) Optimizing Compiler Version 17.00.50727.1 

include listing.inc

INCLUDELIB LIBCMT
INCLUDELIB OLDNAMES

PUBLIC	?scheduler_enable@@3_NA				; scheduler_enable
PUBLIC	?scheduler_initialized@@3_NA			; scheduler_initialized
PUBLIC	?task_id@@3GA					; task_id
PUBLIC	?task_list_head@@3PEAU_thread_@@EA		; task_list_head
PUBLIC	?task_list_last@@3PEAU_thread_@@EA		; task_list_last
PUBLIC	?blocked_thr_head@@3PEAU_thread_@@EA		; blocked_thr_head
PUBLIC	?blocked_thr_last@@3PEAU_thread_@@EA		; blocked_thr_last
PUBLIC	?system_tick@@3IA				; system_tick
PUBLIC	current_thread
_BSS	SEGMENT
?scheduler_enable@@3_NA DB 01H DUP (?)			; scheduler_enable
	ALIGN	4

?scheduler_initialized@@3_NA DB 01H DUP (?)		; scheduler_initialized
	ALIGN	4

?task_id@@3GA DW 01H DUP (?)				; task_id
	ALIGN	8

?task_list_head@@3PEAU_thread_@@EA DQ 01H DUP (?)	; task_list_head
?task_list_last@@3PEAU_thread_@@EA DQ 01H DUP (?)	; task_list_last
?blocked_thr_head@@3PEAU_thread_@@EA DQ 01H DUP (?)	; blocked_thr_head
?blocked_thr_last@@3PEAU_thread_@@EA DQ 01H DUP (?)	; blocked_thr_last
?system_tick@@3IA DD 01H DUP (?)			; system_tick
	ALIGN	8

current_thread DQ 01H DUP (?)
_BSS	ENDS
CONST	SEGMENT
$SG3803	DB	'Idle', 00H
CONST	ENDS
PUBLIC	?AuInitializeScheduler@@YAXXZ			; AuInitializeScheduler
PUBLIC	?AuSchedulerStart@@YAXXZ			; AuSchedulerStart
PUBLIC	?create_user_thread@@YAPEAU_thread_@@P6AXPEAX@Z_K2QEADE@Z ; create_user_thread
PUBLIC	create_kthread
PUBLIC	create_child_thread
PUBLIC	block_thread
PUBLIC	unblock_thread
PUBLIC	get_current_thread
PUBLIC	?set_multi_task_enable@@YAX_N@Z			; set_multi_task_enable
PUBLIC	?is_multi_task_enable@@YA_NXZ			; is_multi_task_enable
PUBLIC	force_sched
PUBLIC	?thread_iterate_ready_list@@YAPEAU_thread_@@G@Z	; thread_iterate_ready_list
PUBLIC	?thread_iterate_block_list@@YAPEAU_thread_@@H@Z	; thread_iterate_block_list
PUBLIC	?thread_get_id_by_name@@YAGPEAD@Z		; thread_get_id_by_name
PUBLIC	?set_current_thread@@YAXPEAU_thread_@@@Z	; set_current_thread
PUBLIC	sleep_thread
PUBLIC	?task_delete@@YAXPEAU_thread_@@@Z		; task_delete
PUBLIC	?is_scheduler_initialized@@YA_NXZ		; is_scheduler_initialized
PUBLIC	?sched_get_tick@@YAIXZ				; sched_get_tick
PUBLIC	?thread_insert@@YAXPEAU_thread_@@@Z		; thread_insert
PUBLIC	?thread_insert_block@@YAXPEAU_thread_@@@Z	; thread_insert_block
PUBLIC	?thread_delete_block@@YAXPEAU_thread_@@@Z	; thread_delete_block
PUBLIC	?idle_thread@@YAXXZ				; idle_thread
PUBLIC	?next_task@@YAXPEAU_cpu_@@@Z			; next_task
PUBLIC	?sig_loop@@YAXXZ				; sig_loop
PUBLIC	?scheduler_isr@@YAX_KPEAX@Z			; scheduler_isr
EXTRN	strncpy:PROC
EXTRN	memset:PROC
EXTRN	AuPmmngrAlloc:PROC
EXTRN	p2v:PROC
EXTRN	v2p:PROC
EXTRN	x64_cli:PROC
EXTRN	x64_read_cr3:PROC
EXTRN	x64_fxsave:PROC
EXTRN	x64_fxrstor:PROC
EXTRN	x64_ldmxcsr:PROC
EXTRN	x64_set_kstack:PROC
EXTRN	x64_get_kstack:PROC
EXTRN	setvect:PROC
EXTRN	?x86_64_cpu_get_id@@YAEXZ:PROC			; x86_64_cpu_get_id
EXTRN	?is_cpu_fxsave_supported@@YA_NXZ:PROC		; is_cpu_fxsave_supported
EXTRN	?AuMapPageEx@@YA_NPEA_K_K1E@Z:PROC		; AuMapPageEx
EXTRN	malloc:PROC
EXTRN	save_context:PROC
EXTRN	execute_idle:PROC
EXTRN	get_kernel_tss:PROC
EXTRN	?allocate_kstack@@YA_KPEA_K@Z:PROC		; allocate_kstack
EXTRN	?allocate_kstack_child@@YA_KPEA_K@Z:PROC	; allocate_kstack_child
EXTRN	?x86_64_check_signal@@YA_NPEAU_thread_@@PEAUinterrupt_stack_frame@@@Z:PROC ; x86_64_check_signal
EXTRN	?x86_64_get_signal@@YAPEAU_signal_@@PEAU_thread_@@@Z:PROC ; x86_64_get_signal
EXTRN	?x86_64_prepare_signal@@YAXPEAU_thread_@@PEAUinterrupt_stack_frame@@PEAU_signal_@@@Z:PROC ; x86_64_prepare_signal
EXTRN	?timer_fire@@YAXXZ:PROC				; timer_fire
EXTRN	apic_local_eoi:PROC
EXTRN	?AuPCPUGetCpu@@YAPEAU_cpu_@@E@Z:PROC		; AuPCPUGetCpu
EXTRN	?AuPCPUSetCurrentThread@@YAXPEAU_thread_@@@Z:PROC ; AuPCPUSetCurrentThread
EXTRN	force_sched_apic:PROC
pdata	SEGMENT
$pdata$?AuInitializeScheduler@@YAXXZ DD imagerel $LN3
	DD	imagerel $LN3+125
	DD	imagerel $unwind$?AuInitializeScheduler@@YAXXZ
$pdata$?AuSchedulerStart@@YAXXZ DD imagerel $LN3
	DD	imagerel $LN3+46
	DD	imagerel $unwind$?AuSchedulerStart@@YAXXZ
$pdata$?create_user_thread@@YAPEAU_thread_@@P6AXPEAX@Z_K2QEADE@Z DD imagerel $LN3
	DD	imagerel $LN3+836
	DD	imagerel $unwind$?create_user_thread@@YAPEAU_thread_@@P6AXPEAX@Z_K2QEADE@Z
$pdata$create_kthread DD imagerel $LN3
	DD	imagerel $LN3+721
	DD	imagerel $unwind$create_kthread
$pdata$create_child_thread DD imagerel $LN3
	DD	imagerel $LN3+745
	DD	imagerel $unwind$create_child_thread
$pdata$block_thread DD imagerel $LN3
	DD	imagerel $LN3+46
	DD	imagerel $unwind$block_thread
$pdata$unblock_thread DD imagerel $LN7
	DD	imagerel $LN7+99
	DD	imagerel $unwind$unblock_thread
$pdata$force_sched DD imagerel $LN3
	DD	imagerel $LN3+14
	DD	imagerel $unwind$force_sched
$pdata$?thread_iterate_ready_list@@YAPEAU_thread_@@G@Z DD imagerel $LN7
	DD	imagerel $LN7+79
	DD	imagerel $unwind$?thread_iterate_ready_list@@YAPEAU_thread_@@G@Z
$pdata$?thread_iterate_block_list@@YAPEAU_thread_@@H@Z DD imagerel $LN8
	DD	imagerel $LN8+85
	DD	imagerel $unwind$?thread_iterate_block_list@@YAPEAU_thread_@@H@Z
$pdata$?thread_get_id_by_name@@YAGPEAD@Z DD imagerel $LN14
	DD	imagerel $LN14+198
	DD	imagerel $unwind$?thread_get_id_by_name@@YAGPEAD@Z
$pdata$?next_task@@YAXPEAU_cpu_@@@Z DD imagerel $LN9
	DD	imagerel $LN9+172
	DD	imagerel $unwind$?next_task@@YAXPEAU_cpu_@@@Z
$pdata$?sig_loop@@YAXXZ DD imagerel $LN5
	DD	imagerel $LN5+16
	DD	imagerel $unwind$?sig_loop@@YAXXZ
$pdata$?scheduler_isr@@YAX_KPEAX@Z DD imagerel $LN11
	DD	imagerel $LN11+419
	DD	imagerel $unwind$?scheduler_isr@@YAX_KPEAX@Z
pdata	ENDS
xdata	SEGMENT
$unwind$?AuInitializeScheduler@@YAXXZ DD 010401H
	DD	08204H
$unwind$?AuSchedulerStart@@YAXXZ DD 010401H
	DD	04204H
$unwind$?create_user_thread@@YAPEAU_thread_@@P6AXPEAX@Z_K2QEADE@Z DD 011801H
	DD	06218H
$unwind$create_kthread DD 011801H
	DD	06218H
$unwind$create_child_thread DD 011801H
	DD	06218H
$unwind$block_thread DD 010901H
	DD	04209H
$unwind$unblock_thread DD 010901H
	DD	06209H
$unwind$force_sched DD 010401H
	DD	04204H
$unwind$?thread_iterate_ready_list@@YAPEAU_thread_@@G@Z DD 010901H
	DD	02209H
$unwind$?thread_iterate_block_list@@YAPEAU_thread_@@H@Z DD 010801H
	DD	02208H
$unwind$?thread_get_id_by_name@@YAGPEAD@Z DD 010901H
	DD	04209H
$unwind$?next_task@@YAXPEAU_cpu_@@@Z DD 010901H
	DD	02209H
$unwind$?sig_loop@@YAXXZ DD 010401H
	DD	04204H
$unwind$?scheduler_isr@@YAX_KPEAX@Z DD 010e01H
	DD	0820eH
xdata	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
ktss$ = 32
frame$ = 40
pcpu$ = 48
sig$1 = 56
v$ = 80
param$ = 88
?scheduler_isr@@YAX_KPEAX@Z PROC			; scheduler_isr

; 407  : void scheduler_isr (size_t v, void* param) {

$LN11:
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	sub	rsp, 72					; 00000048H

; 408  : 	x64_cli();

	call	x64_cli

; 409  : 	interrupt_stack_frame *frame = (interrupt_stack_frame*)param;

	mov	rax, QWORD PTR param$[rsp]
	mov	QWORD PTR frame$[rsp], rax

; 410  : 	
; 411  : 	/* check for enable bit, if yes than proceed for 
; 412  : 	   multitasking */
; 413  : 	if (scheduler_enable == false)

	movzx	eax, BYTE PTR ?scheduler_enable@@3_NA	; scheduler_enable
	test	eax, eax
	jne	SHORT $LN8@scheduler_

; 414  : 		goto sched_end;

	jmp	$LN7@scheduler_
	jmp	$sched_end$12
$LN8@scheduler_:

; 415  : 	
; 416  : 	cpu_t* pcpu = AuPCPUGetCpu(x86_64_cpu_get_id());

	call	?x86_64_cpu_get_id@@YAEXZ		; x86_64_cpu_get_id
	movzx	ecx, al
	call	?AuPCPUGetCpu@@YAPEAU_cpu_@@E@Z		; AuPCPUGetCpu
	mov	QWORD PTR pcpu$[rsp], rax

; 417  : 	TSS *ktss = pcpu->kernel_tss;

	mov	rax, QWORD PTR pcpu$[rsp]
	mov	rax, QWORD PTR [rax+9]
	mov	QWORD PTR ktss$[rsp], rax

; 418  : 	//mutex_lock (scheduler_mutex);
; 419  : 	/** save currently running thread contexts */
; 420  : 	if (save_context(current_thread,ktss) == 0) {

	mov	rdx, QWORD PTR ktss$[rsp]
	mov	rcx, QWORD PTR current_thread
	call	save_context
	test	eax, eax
	jne	$LN6@scheduler_

; 421  : 		current_thread->frame.cr3 = x64_read_cr3();

	call	x64_read_cr3
	mov	rcx, QWORD PTR current_thread
	mov	QWORD PTR [rcx+192], rax

; 422  : 
; 423  : 		
; 424  : 		/* check if the thread is user mode thread, if yes
; 425  : 		   than store the kernel esp */
; 426  : 		if (current_thread->priviledge == THREAD_LEVEL_USER) {

	mov	rax, QWORD PTR current_thread
	movzx	eax, BYTE PTR [rax+240]
	cmp	eax, 2
	jne	SHORT $LN5@scheduler_

; 427  : 			//current_thread->kern_esp = ktss->rsp[0];
; 428  : 			current_thread->frame.kern_esp = x64_get_kstack(ktss);

	mov	rcx, QWORD PTR ktss$[rsp]
	call	x64_get_kstack
	mov	rcx, QWORD PTR current_thread
	mov	QWORD PTR [rcx+200], rax
$LN5@scheduler_:

; 429  : 		}
; 430  : 
; 431  : 
; 432  : 		/* 
; 433  : 		 * check for pending signal and make current_thread prepared
; 434  : 		 * for handling the signal (((**********BBBBBUUUUUGGGGGGYYYYYYY*******)))
; 435  : 		 */
; 436  : 		if (x86_64_check_signal(current_thread, frame)) {

	mov	rdx, QWORD PTR frame$[rsp]
	mov	rcx, QWORD PTR current_thread
	call	?x86_64_check_signal@@YA_NPEAU_thread_@@PEAUinterrupt_stack_frame@@@Z ; x86_64_check_signal
	movzx	eax, al
	test	eax, eax
	je	SHORT $LN4@scheduler_

; 437  : 			signal_t *sig = x86_64_get_signal(current_thread);

	mov	rcx, QWORD PTR current_thread
	call	?x86_64_get_signal@@YAPEAU_signal_@@PEAU_thread_@@@Z ; x86_64_get_signal
	mov	QWORD PTR sig$1[rsp], rax

; 438  : 			x86_64_prepare_signal(current_thread,frame,sig);

	mov	r8, QWORD PTR sig$1[rsp]
	mov	rdx, QWORD PTR frame$[rsp]
	mov	rcx, QWORD PTR current_thread
	call	?x86_64_prepare_signal@@YAXPEAU_thread_@@PEAUinterrupt_stack_frame@@PEAU_signal_@@@Z ; x86_64_prepare_signal
$LN4@scheduler_:

; 439  : 		}
; 440  : 
; 441  : 
; 442  : 		if (is_cpu_fxsave_supported())

	call	?is_cpu_fxsave_supported@@YA_NXZ	; is_cpu_fxsave_supported
	movzx	eax, al
	test	eax, eax
	je	SHORT $LN3@scheduler_

; 443  : 			x64_fxsave(current_thread->fx_state);

	mov	rax, QWORD PTR current_thread
	mov	rcx, QWORD PTR [rax+216]
	call	x64_fxsave
$LN3@scheduler_:

; 444  : 
; 445  : 		/* now get the next runnable task from the thread list */
; 446  : 		next_task(pcpu);

	mov	rcx, QWORD PTR pcpu$[rsp]
	call	?next_task@@YAXPEAU_cpu_@@@Z		; next_task

; 447  : 
; 448  : 		/*
; 449  : 		  Here increase the system tick and
; 450  : 		  fire the timer
; 451  : 		  */
; 452  : 		system_tick++;

	mov	eax, DWORD PTR ?system_tick@@3IA	; system_tick
	inc	eax
	mov	DWORD PTR ?system_tick@@3IA, eax	; system_tick

; 453  : 		timer_fire();

	call	?timer_fire@@YAXXZ			; timer_fire

; 454  : #ifdef USE_APIC
; 455  : 	    apic_local_eoi();

	call	apic_local_eoi

; 456  : #endif
; 457  : #ifdef USE_PIC
; 458  : 		AuInterruptEnd (0);
; 459  : #endif
; 460  : 		
; 461  : 		/** now return to the new task last stored instruction */
; 462  : 
; 463  : 
; 464  : 		if (current_thread->priviledge == THREAD_LEVEL_USER){

	mov	rax, QWORD PTR current_thread
	movzx	eax, BYTE PTR [rax+240]
	cmp	eax, 2
	jne	SHORT $LN2@scheduler_

; 465  : 			//ktss->rsp[0] = current_thread->kern_esp;
; 466  : 			x64_set_kstack(ktss,current_thread->frame.kern_esp);

	mov	rax, QWORD PTR current_thread
	mov	rdx, QWORD PTR [rax+200]
	mov	rcx, QWORD PTR ktss$[rsp]
	call	x64_set_kstack
$LN2@scheduler_:

; 467  : 		}
; 468  : 
; 469  : 		if (is_cpu_fxsave_supported())

	call	?is_cpu_fxsave_supported@@YA_NXZ	; is_cpu_fxsave_supported
	movzx	eax, al
	test	eax, eax
	je	SHORT $LN1@scheduler_

; 470  : 			x64_fxrstor(current_thread->fx_state);

	mov	rax, QWORD PTR current_thread
	mov	rcx, QWORD PTR [rax+216]
	call	x64_fxrstor
$LN1@scheduler_:

; 471  : 
; 472  : 		x64_ldmxcsr(&current_thread->mxcsr);

	mov	rax, QWORD PTR current_thread
	add	rax, 224				; 000000e0H
	mov	rcx, rax
	call	x64_ldmxcsr

; 473  : 
; 474  : 
; 475  : 		//
; 476  : 		//mutex_unlock (scheduler_mutex);
; 477  : 		execute_idle (current_thread,ktss);

	mov	rdx, QWORD PTR ktss$[rsp]
	mov	rcx, QWORD PTR current_thread
	call	execute_idle
$LN6@scheduler_:
$LN7@scheduler_:
$sched_end$12:

; 478  : 		
; 479  : 	}
; 480  : 
; 481  : sched_end:
; 482  : #ifdef USE_APIC
; 483  : 	apic_local_eoi();

	call	apic_local_eoi

; 484  : #endif
; 485  : #ifdef USE_PIC
; 486  : 	AuInterruptEnd(0);
; 487  : #endif
; 488  : }

	add	rsp, 72					; 00000048H
	ret	0
?scheduler_isr@@YAX_KPEAX@Z ENDP			; scheduler_isr
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
?sig_loop@@YAXXZ PROC					; sig_loop

; 396  : void sig_loop () {

$LN5:
	sub	rsp, 40					; 00000028H
$LN2@sig_loop:

; 397  : 	for(;;) {
; 398  : 		//_debug_print_ ("Loop    ");
; 399  : 		x64_cli();

	call	x64_cli

; 400  : 	}

	jmp	SHORT $LN2@sig_loop

; 401  : }

	add	rsp, 40					; 00000028H
	ret	0
?sig_loop@@YAXXZ ENDP					; sig_loop
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
task$ = 0
pcpu$ = 32
?next_task@@YAXPEAU_cpu_@@@Z PROC			; next_task

; 372  : void next_task (cpu_t *pcpu) {

$LN9:
	mov	QWORD PTR [rsp+8], rcx
	sub	rsp, 24

; 373  : 	thread_t* task = (thread_t*)pcpu->au_current_thread;//current_thread;

	mov	rax, QWORD PTR pcpu$[rsp]
	mov	rax, QWORD PTR [rax+1]
	mov	QWORD PTR task$[rsp], rax
$LN6@next_task:

; 374  : 	do {
; 375  : 		if (task->state == THREAD_STATE_SLEEP) {

	mov	rax, QWORD PTR task$[rsp]
	movzx	eax, BYTE PTR [rax+237]
	cmp	eax, 4
	jne	SHORT $LN3@next_task

; 376  : 			if (task->quanta == 0) {

	mov	rax, QWORD PTR task$[rsp]
	movzx	eax, WORD PTR [rax+242]
	test	eax, eax
	jne	SHORT $LN2@next_task

; 377  : 				task->state = THREAD_STATE_READY;

	mov	rax, QWORD PTR task$[rsp]
	mov	BYTE PTR [rax+237], 1
$LN2@next_task:

; 378  : 			}
; 379  : 			task->quanta--;

	mov	rax, QWORD PTR task$[rsp]
	movzx	eax, WORD PTR [rax+242]
	dec	ax
	mov	rcx, QWORD PTR task$[rsp]
	mov	WORD PTR [rcx+242], ax
$LN3@next_task:

; 380  : 		}
; 381  : 		task = task->next;

	mov	rax, QWORD PTR task$[rsp]
	mov	rax, QWORD PTR [rax+1088]
	mov	QWORD PTR task$[rsp], rax

; 382  : 		if (task == NULL) {

	cmp	QWORD PTR task$[rsp], 0
	jne	SHORT $LN1@next_task

; 383  : 			task = task_list_head;

	mov	rax, QWORD PTR ?task_list_head@@3PEAU_thread_@@EA ; task_list_head
	mov	QWORD PTR task$[rsp], rax
$LN1@next_task:

; 384  : 		}
; 385  : 	}while (task->state != THREAD_STATE_READY);

	mov	rax, QWORD PTR task$[rsp]
	movzx	eax, BYTE PTR [rax+237]
	cmp	eax, 1
	jne	SHORT $LN6@next_task
$end$10:

; 386  : 
; 387  : end:
; 388  : 	//AuPCPUSetCurrentThread(task);
; 389  : 	pcpu->au_current_thread = (uint64_t*)task;

	mov	rax, QWORD PTR pcpu$[rsp]
	mov	rcx, QWORD PTR task$[rsp]
	mov	QWORD PTR [rax+1], rcx

; 390  : 	current_thread = (thread_t*)pcpu->au_current_thread;//AuPCPUGetCurrentThread(); //task;

	mov	rax, QWORD PTR pcpu$[rsp]
	mov	rax, QWORD PTR [rax+1]
	mov	QWORD PTR current_thread, rax

; 391  : }

	add	rsp, 24
	ret	0
?next_task@@YAXPEAU_cpu_@@@Z ENDP			; next_task
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
?idle_thread@@YAXXZ PROC				; idle_thread

; 341  : void idle_thread () {

	npad	2
$LN2@idle_threa:

; 342  : 	while(1) {

	xor	eax, eax
	cmp	eax, 1
	je	SHORT $LN1@idle_threa

; 343  : 		//x64_hlt();
; 344  : 	}

	jmp	SHORT $LN2@idle_threa
$LN1@idle_threa:

; 345  : }

	fatret	0
?idle_thread@@YAXXZ ENDP				; idle_thread
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
thread$ = 8
?thread_delete_block@@YAXPEAU_thread_@@@Z PROC		; thread_delete_block

; 140  : void thread_delete_block (thread_t* thread) {

	mov	QWORD PTR [rsp+8], rcx

; 141  : 
; 142  : 	if (blocked_thr_head == NULL)

	cmp	QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA, 0 ; blocked_thr_head
	jne	SHORT $LN5@thread_del

; 143  : 		return;

	jmp	$LN6@thread_del
$LN5@thread_del:

; 144  : 
; 145  : 	if (thread == blocked_thr_head) {

	mov	rax, QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA ; blocked_thr_head
	cmp	QWORD PTR thread$[rsp], rax
	jne	SHORT $LN4@thread_del

; 146  : 		blocked_thr_head = blocked_thr_head->next;

	mov	rax, QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA ; blocked_thr_head
	mov	rax, QWORD PTR [rax+1088]
	mov	QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA, rax ; blocked_thr_head

; 147  : 	} else {

	jmp	SHORT $LN3@thread_del
$LN4@thread_del:

; 148  : 		thread->prev->next = thread->next;

	mov	rax, QWORD PTR thread$[rsp]
	mov	rax, QWORD PTR [rax+1096]
	mov	rcx, QWORD PTR thread$[rsp]
	mov	rcx, QWORD PTR [rcx+1088]
	mov	QWORD PTR [rax+1088], rcx
$LN3@thread_del:

; 149  : 	}
; 150  : 
; 151  : 	if (thread == blocked_thr_last) {

	mov	rax, QWORD PTR ?blocked_thr_last@@3PEAU_thread_@@EA ; blocked_thr_last
	cmp	QWORD PTR thread$[rsp], rax
	jne	SHORT $LN2@thread_del

; 152  : 		blocked_thr_last = thread->prev;

	mov	rax, QWORD PTR thread$[rsp]
	mov	rax, QWORD PTR [rax+1096]
	mov	QWORD PTR ?blocked_thr_last@@3PEAU_thread_@@EA, rax ; blocked_thr_last

; 153  : 	} else {

	jmp	SHORT $LN1@thread_del
$LN2@thread_del:

; 154  : 		thread->next->prev = thread->prev;

	mov	rax, QWORD PTR thread$[rsp]
	mov	rax, QWORD PTR [rax+1088]
	mov	rcx, QWORD PTR thread$[rsp]
	mov	rcx, QWORD PTR [rcx+1096]
	mov	QWORD PTR [rax+1096], rcx
$LN1@thread_del:
$LN6@thread_del:

; 155  : 	}
; 156  : }

	fatret	0
?thread_delete_block@@YAXPEAU_thread_@@@Z ENDP		; thread_delete_block
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
new_task$ = 8
?thread_insert_block@@YAXPEAU_thread_@@@Z PROC		; thread_insert_block

; 122  : void thread_insert_block (thread_t* new_task ) {

	mov	QWORD PTR [rsp+8], rcx

; 123  : 	new_task->next = NULL;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	QWORD PTR [rax+1088], 0

; 124  : 	new_task->prev = NULL;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	QWORD PTR [rax+1096], 0

; 125  : 
; 126  : 	if (blocked_thr_head == NULL) {

	cmp	QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA, 0 ; blocked_thr_head
	jne	SHORT $LN2@thread_ins

; 127  : 		blocked_thr_last = new_task;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	QWORD PTR ?blocked_thr_last@@3PEAU_thread_@@EA, rax ; blocked_thr_last

; 128  : 		blocked_thr_head = new_task;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA, rax ; blocked_thr_head

; 129  : 	} else {

	jmp	SHORT $LN1@thread_ins
$LN2@thread_ins:

; 130  : 		blocked_thr_last->next = new_task;

	mov	rax, QWORD PTR ?blocked_thr_last@@3PEAU_thread_@@EA ; blocked_thr_last
	mov	rcx, QWORD PTR new_task$[rsp]
	mov	QWORD PTR [rax+1088], rcx

; 131  : 		new_task->prev = blocked_thr_last;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	rcx, QWORD PTR ?blocked_thr_last@@3PEAU_thread_@@EA ; blocked_thr_last
	mov	QWORD PTR [rax+1096], rcx
$LN1@thread_ins:

; 132  : 	}
; 133  : 	blocked_thr_last = new_task;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	QWORD PTR ?blocked_thr_last@@3PEAU_thread_@@EA, rax ; blocked_thr_last

; 134  : }

	ret	0
?thread_insert_block@@YAXPEAU_thread_@@@Z ENDP		; thread_insert_block
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
new_task$ = 8
?thread_insert@@YAXPEAU_thread_@@@Z PROC		; thread_insert

; 74   : void thread_insert (thread_t* new_task ) {

	mov	QWORD PTR [rsp+8], rcx

; 75   : 	new_task->next = NULL;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	QWORD PTR [rax+1088], 0

; 76   : 	new_task->prev = NULL;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	QWORD PTR [rax+1096], 0

; 77   : 
; 78   : 	if (task_list_head == NULL) {

	cmp	QWORD PTR ?task_list_head@@3PEAU_thread_@@EA, 0 ; task_list_head
	jne	SHORT $LN2@thread_ins

; 79   : 		task_list_last = new_task;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	QWORD PTR ?task_list_last@@3PEAU_thread_@@EA, rax ; task_list_last

; 80   : 		task_list_head = new_task;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	QWORD PTR ?task_list_head@@3PEAU_thread_@@EA, rax ; task_list_head

; 81   : 		current_thread = task_list_last;

	mov	rax, QWORD PTR ?task_list_last@@3PEAU_thread_@@EA ; task_list_last
	mov	QWORD PTR current_thread, rax

; 82   : 	} else {

	jmp	SHORT $LN1@thread_ins
$LN2@thread_ins:

; 83   : 		task_list_last->next = new_task;

	mov	rax, QWORD PTR ?task_list_last@@3PEAU_thread_@@EA ; task_list_last
	mov	rcx, QWORD PTR new_task$[rsp]
	mov	QWORD PTR [rax+1088], rcx

; 84   : 		new_task->prev = task_list_last;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	rcx, QWORD PTR ?task_list_last@@3PEAU_thread_@@EA ; task_list_last
	mov	QWORD PTR [rax+1096], rcx
$LN1@thread_ins:

; 85   : 	}
; 86   : 	task_list_last = new_task;

	mov	rax, QWORD PTR new_task$[rsp]
	mov	QWORD PTR ?task_list_last@@3PEAU_thread_@@EA, rax ; task_list_last

; 87   : }

	ret	0
?thread_insert@@YAXPEAU_thread_@@@Z ENDP		; thread_insert
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
?sched_get_tick@@YAIXZ PROC				; sched_get_tick

; 625  : 	return system_tick;

	mov	eax, DWORD PTR ?system_tick@@3IA	; system_tick

; 626  : }

	ret	0
?sched_get_tick@@YAIXZ ENDP				; sched_get_tick
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
?is_scheduler_initialized@@YA_NXZ PROC			; is_scheduler_initialized

; 618  : 	return scheduler_initialized;

	movzx	eax, BYTE PTR ?scheduler_initialized@@3_NA ; scheduler_initialized

; 619  : }

	ret	0
?is_scheduler_initialized@@YA_NXZ ENDP			; is_scheduler_initialized
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
thread$ = 8
?task_delete@@YAXPEAU_thread_@@@Z PROC			; task_delete

; 93   : void task_delete (thread_t* thread) {

	mov	QWORD PTR [rsp+8], rcx

; 94   : 
; 95   : 	if (task_list_head == NULL)

	cmp	QWORD PTR ?task_list_head@@3PEAU_thread_@@EA, 0 ; task_list_head
	jne	SHORT $LN5@task_delet

; 96   : 		return;

	jmp	$LN6@task_delet
$LN5@task_delet:

; 97   : 
; 98   : 	if (thread == task_list_head) {

	mov	rax, QWORD PTR ?task_list_head@@3PEAU_thread_@@EA ; task_list_head
	cmp	QWORD PTR thread$[rsp], rax
	jne	SHORT $LN4@task_delet

; 99   : 		task_list_head = task_list_head->next;

	mov	rax, QWORD PTR ?task_list_head@@3PEAU_thread_@@EA ; task_list_head
	mov	rax, QWORD PTR [rax+1088]
	mov	QWORD PTR ?task_list_head@@3PEAU_thread_@@EA, rax ; task_list_head

; 100  : 	} else {

	jmp	SHORT $LN3@task_delet
$LN4@task_delet:

; 101  : 		thread->prev->next = thread->next;

	mov	rax, QWORD PTR thread$[rsp]
	mov	rax, QWORD PTR [rax+1096]
	mov	rcx, QWORD PTR thread$[rsp]
	mov	rcx, QWORD PTR [rcx+1088]
	mov	QWORD PTR [rax+1088], rcx
$LN3@task_delet:

; 102  : 	}
; 103  : 
; 104  : 	if (thread == task_list_last) {

	mov	rax, QWORD PTR ?task_list_last@@3PEAU_thread_@@EA ; task_list_last
	cmp	QWORD PTR thread$[rsp], rax
	jne	SHORT $LN2@task_delet

; 105  : 		task_list_last = thread->prev;

	mov	rax, QWORD PTR thread$[rsp]
	mov	rax, QWORD PTR [rax+1096]
	mov	QWORD PTR ?task_list_last@@3PEAU_thread_@@EA, rax ; task_list_last

; 106  : 	} else {

	jmp	SHORT $LN1@task_delet
$LN2@task_delet:

; 107  : 		thread->next->prev = thread->prev;

	mov	rax, QWORD PTR thread$[rsp]
	mov	rax, QWORD PTR [rax+1088]
	mov	rcx, QWORD PTR thread$[rsp]
	mov	rcx, QWORD PTR [rcx+1096]
	mov	QWORD PTR [rax+1096], rcx
$LN1@task_delet:
$LN6@task_delet:

; 108  : 	}
; 109  : 
; 110  : 	/* donot free the thread, cuz when thread needs
; 111  : 	 * to move from runnable queue to blocked queue
; 112  : 	 * same address is used, rather call 'free' 
; 113  : 	 * externally 
; 114  : 	 */
; 115  : }

	fatret	0
?task_delete@@YAXPEAU_thread_@@@Z ENDP			; task_delete
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
t$ = 8
ms$ = 16
sleep_thread PROC

; 594  : void sleep_thread (thread_t *t, uint64_t ms) {

	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx

; 595  : 	t->quanta = ms;

	mov	rax, QWORD PTR t$[rsp]
	movzx	ecx, WORD PTR ms$[rsp]
	mov	WORD PTR [rax+242], cx

; 596  : 	t->state = THREAD_STATE_SLEEP;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+237], 4

; 597  : }

	ret	0
sleep_thread ENDP
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
thread$ = 8
?set_current_thread@@YAXPEAU_thread_@@@Z PROC		; set_current_thread

; 511  : void set_current_thread (thread_t *thread) {

	mov	QWORD PTR [rsp+8], rcx

; 512  : 	current_thread = thread;

	mov	rax, QWORD PTR thread$[rsp]
	mov	QWORD PTR current_thread, rax

; 513  : }

	ret	0
?set_current_thread@@YAXPEAU_thread_@@@Z ENDP		; set_current_thread
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
id$ = 0
it$1 = 8
thr$2 = 16
name$ = 48
?thread_get_id_by_name@@YAGPEAD@Z PROC			; thread_get_id_by_name

; 555  : uint16_t thread_get_id_by_name (char* name) {

$LN14:
	mov	QWORD PTR [rsp+8], rcx
	sub	rsp, 40					; 00000028H

; 556  : 	uint16_t id = 0;

	xor	eax, eax
	mov	WORD PTR id$[rsp], ax

; 557  : 	for (thread_t *it = task_list_head; it != NULL; it = it->next) {

	mov	rax, QWORD PTR ?task_list_head@@3PEAU_thread_@@EA ; task_list_head
	mov	QWORD PTR it$1[rsp], rax
	jmp	SHORT $LN11@thread_get
$LN10@thread_get:
	mov	rax, QWORD PTR it$1[rsp]
	mov	rax, QWORD PTR [rax+1088]
	mov	QWORD PTR it$1[rsp], rax
$LN11@thread_get:
	cmp	QWORD PTR it$1[rsp], 0
	je	SHORT $LN9@thread_get

; 558  : 		if (it->name == name) {

	mov	rax, QWORD PTR it$1[rsp]
	add	rax, 229				; 000000e5H
	cmp	rax, QWORD PTR name$[rsp]
	jne	SHORT $LN8@thread_get

; 559  : 			id = it->id;

	mov	rax, QWORD PTR it$1[rsp]
	movzx	eax, WORD PTR [rax+238]
	mov	WORD PTR id$[rsp], ax

; 560  : 			break;

	jmp	SHORT $LN9@thread_get
$LN8@thread_get:

; 561  : 		}
; 562  : 	}

	jmp	SHORT $LN10@thread_get
$LN9@thread_get:

; 563  : 
; 564  : 	if (id > 0)

	movzx	eax, WORD PTR id$[rsp]
	test	eax, eax
	jle	SHORT $LN7@thread_get

; 565  : 		return id;

	movzx	eax, WORD PTR id$[rsp]
	jmp	SHORT $LN12@thread_get

; 566  : 	else{

	jmp	SHORT $LN6@thread_get
$LN7@thread_get:

; 567  : 		if (blocked_thr_head != NULL)

	cmp	QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA, 0 ; blocked_thr_head
	je	SHORT $LN5@thread_get

; 568  : 			for (thread_t *thr = blocked_thr_head; thr != NULL; thr = thr->next) {

	mov	rax, QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA ; blocked_thr_head
	mov	QWORD PTR thr$2[rsp], rax
	jmp	SHORT $LN4@thread_get
$LN3@thread_get:
	mov	rax, QWORD PTR thr$2[rsp]
	mov	rax, QWORD PTR [rax+1088]
	mov	QWORD PTR thr$2[rsp], rax
$LN4@thread_get:
	cmp	QWORD PTR thr$2[rsp], 0
	je	SHORT $LN2@thread_get

; 569  : 				if (thr->name == name) 

	mov	rax, QWORD PTR thr$2[rsp]
	add	rax, 229				; 000000e5H
	cmp	rax, QWORD PTR name$[rsp]
	jne	SHORT $LN1@thread_get

; 570  : 					return thr->id;

	mov	rax, QWORD PTR thr$2[rsp]
	movzx	eax, WORD PTR [rax+238]
	jmp	SHORT $LN12@thread_get
$LN1@thread_get:

; 571  : 			}

	jmp	SHORT $LN3@thread_get
$LN2@thread_get:
$LN5@thread_get:
$LN6@thread_get:

; 572  : 	}
; 573  : 
; 574  : 	return NULL;

	xor	eax, eax
$LN12@thread_get:

; 575  : }

	add	rsp, 40					; 00000028H
	ret	0
?thread_get_id_by_name@@YAGPEAD@Z ENDP			; thread_get_id_by_name
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
thr$1 = 0
id$ = 32
?thread_iterate_block_list@@YAPEAU_thread_@@H@Z PROC	; thread_iterate_block_list

; 534  : thread_t* thread_iterate_block_list (int id) {

$LN8:
	mov	DWORD PTR [rsp+8], ecx
	sub	rsp, 24

; 535  : 	if (blocked_thr_head != NULL)

	cmp	QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA, 0 ; blocked_thr_head
	je	SHORT $LN5@thread_ite

; 536  : 		for (thread_t *thr = blocked_thr_head; thr != NULL; thr = thr->next) {

	mov	rax, QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA ; blocked_thr_head
	mov	QWORD PTR thr$1[rsp], rax
	jmp	SHORT $LN4@thread_ite
$LN3@thread_ite:
	mov	rax, QWORD PTR thr$1[rsp]
	mov	rax, QWORD PTR [rax+1088]
	mov	QWORD PTR thr$1[rsp], rax
$LN4@thread_ite:
	cmp	QWORD PTR thr$1[rsp], 0
	je	SHORT $LN2@thread_ite

; 537  : 			if (thr->id == id) {

	mov	rax, QWORD PTR thr$1[rsp]
	movzx	eax, WORD PTR [rax+238]
	cmp	eax, DWORD PTR id$[rsp]
	jne	SHORT $LN1@thread_ite

; 538  : 				return thr;

	mov	rax, QWORD PTR thr$1[rsp]
	jmp	SHORT $LN6@thread_ite
$LN1@thread_ite:

; 539  : 		}
; 540  : 	}

	jmp	SHORT $LN3@thread_ite
$LN2@thread_ite:
$LN5@thread_ite:

; 541  : 	return NULL;

	xor	eax, eax
$LN6@thread_ite:

; 542  : }

	add	rsp, 24
	ret	0
?thread_iterate_block_list@@YAPEAU_thread_@@H@Z ENDP	; thread_iterate_block_list
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
it$1 = 0
id$ = 32
?thread_iterate_ready_list@@YAPEAU_thread_@@G@Z PROC	; thread_iterate_ready_list

; 545  : thread_t * thread_iterate_ready_list (uint16_t id) {

$LN7:
	mov	WORD PTR [rsp+8], cx
	sub	rsp, 24

; 546  : 	for (thread_t *it = task_list_head; it != NULL; it = it->next) {

	mov	rax, QWORD PTR ?task_list_head@@3PEAU_thread_@@EA ; task_list_head
	mov	QWORD PTR it$1[rsp], rax
	jmp	SHORT $LN4@thread_ite
$LN3@thread_ite:
	mov	rax, QWORD PTR it$1[rsp]
	mov	rax, QWORD PTR [rax+1088]
	mov	QWORD PTR it$1[rsp], rax
$LN4@thread_ite:
	cmp	QWORD PTR it$1[rsp], 0
	je	SHORT $LN2@thread_ite

; 547  : 		if (it->id == id) {

	mov	rax, QWORD PTR it$1[rsp]
	movzx	eax, WORD PTR [rax+238]
	movzx	ecx, WORD PTR id$[rsp]
	cmp	eax, ecx
	jne	SHORT $LN1@thread_ite

; 548  : 			return it;

	mov	rax, QWORD PTR it$1[rsp]
	jmp	SHORT $LN5@thread_ite
$LN1@thread_ite:

; 549  : 		}
; 550  : 	}

	jmp	SHORT $LN3@thread_ite
$LN2@thread_ite:

; 551  : 	return NULL;

	xor	eax, eax
$LN5@thread_ite:

; 552  : }

	add	rsp, 24
	ret	0
?thread_iterate_ready_list@@YAPEAU_thread_@@G@Z ENDP	; thread_iterate_ready_list
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
force_sched PROC

; 605  : void force_sched () {

$LN3:
	sub	rsp, 40					; 00000028H

; 606  : #ifdef USE_APIC
; 607  : 	force_sched_apic();

	call	force_sched_apic

; 608  : #elif USE_PIC
; 609  : 	force_sched_pic();
; 610  : #endif
; 611  : }

	add	rsp, 40					; 00000028H
	ret	0
force_sched ENDP
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
?is_multi_task_enable@@YA_NXZ PROC			; is_multi_task_enable

; 519  : 	return scheduler_enable;

	movzx	eax, BYTE PTR ?scheduler_enable@@3_NA	; scheduler_enable

; 520  : }

	ret	0
?is_multi_task_enable@@YA_NXZ ENDP			; is_multi_task_enable
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
value$ = 8
?set_multi_task_enable@@YAX_N@Z PROC			; set_multi_task_enable

; 504  : void set_multi_task_enable (bool value) {

	mov	BYTE PTR [rsp+8], cl

; 505  : 	if (scheduler_enable == value)

	movzx	eax, BYTE PTR ?scheduler_enable@@3_NA	; scheduler_enable
	movzx	ecx, BYTE PTR value$[rsp]
	cmp	eax, ecx
	jne	SHORT $LN1@set_multi_

; 506  : 		return;

	jmp	SHORT $LN2@set_multi_
$LN1@set_multi_:

; 507  : 
; 508  : 	scheduler_enable = value;

	movzx	eax, BYTE PTR value$[rsp]
	mov	BYTE PTR ?scheduler_enable@@3_NA, al	; scheduler_enable
$LN2@set_multi_:

; 509  : }

	fatret	0
?set_multi_task_enable@@YAX_N@Z ENDP			; set_multi_task_enable
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
get_current_thread PROC

; 601  : 	return current_thread;

	mov	rax, QWORD PTR current_thread

; 602  : }

	ret	0
get_current_thread ENDP
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
thr$1 = 32
t$ = 64
unblock_thread PROC

; 578  : AU_EXTERN AU_EXPORT void unblock_thread (thread_t *t) {

$LN7:
	mov	QWORD PTR [rsp+8], rcx
	sub	rsp, 56					; 00000038H

; 579  : 	//x64_cli();
; 580  : 	t->state = THREAD_STATE_READY;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+237], 1

; 581  : 	for (thread_t *thr = blocked_thr_head; thr != NULL; thr = thr->next) {

	mov	rax, QWORD PTR ?blocked_thr_head@@3PEAU_thread_@@EA ; blocked_thr_head
	mov	QWORD PTR thr$1[rsp], rax
	jmp	SHORT $LN4@unblock_th
$LN3@unblock_th:
	mov	rax, QWORD PTR thr$1[rsp]
	mov	rax, QWORD PTR [rax+1088]
	mov	QWORD PTR thr$1[rsp], rax
$LN4@unblock_th:
	cmp	QWORD PTR thr$1[rsp], 0
	je	SHORT $LN2@unblock_th

; 582  : 		if (thr == t) 

	mov	rax, QWORD PTR t$[rsp]
	cmp	QWORD PTR thr$1[rsp], rax
	jne	SHORT $LN1@unblock_th

; 583  : 			thread_delete_block(thr);

	mov	rcx, QWORD PTR thr$1[rsp]
	call	?thread_delete_block@@YAXPEAU_thread_@@@Z ; thread_delete_block
$LN1@unblock_th:

; 584  : 	}

	jmp	SHORT $LN3@unblock_th
$LN2@unblock_th:

; 585  : 	thread_insert (t);

	mov	rcx, QWORD PTR t$[rsp]
	call	?thread_insert@@YAXPEAU_thread_@@@Z	; thread_insert

; 586  : }

	add	rsp, 56					; 00000038H
	ret	0
unblock_thread ENDP
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
thread$ = 48
block_thread PROC

; 524  : AU_EXTERN AU_EXPORT void block_thread (thread_t *thread) {

$LN3:
	mov	QWORD PTR [rsp+8], rcx
	sub	rsp, 40					; 00000028H

; 525  : 	thread->state = THREAD_STATE_BLOCKED;

	mov	rax, QWORD PTR thread$[rsp]
	mov	BYTE PTR [rax+237], 3

; 526  : 	task_delete (thread);

	mov	rcx, QWORD PTR thread$[rsp]
	call	?task_delete@@YAXPEAU_thread_@@@Z	; task_delete

; 527  : 	//list_add (blocked_list,thread);
; 528  : 	thread_insert_block(thread);

	mov	rcx, QWORD PTR thread$[rsp]
	call	?thread_insert_block@@YAXPEAU_thread_@@@Z ; thread_insert_block

; 529  : }

	add	rsp, 40					; 00000028H
	ret	0
block_thread ENDP
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
t$ = 32
parent$ = 64
entry$ = 72
stack$ = 80
name$ = 88
create_child_thread PROC

; 291  : thread_t* create_child_thread (thread_t *parent, void (*entry)(void*),uint64_t stack, char name[8]) {

$LN3:
	mov	QWORD PTR [rsp+32], r9
	mov	QWORD PTR [rsp+24], r8
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	sub	rsp, 56					; 00000038H

; 292  : 	thread_t *t = (thread_t*)malloc(sizeof(thread_t));

	mov	ecx, 1104				; 00000450H
	call	malloc
	mov	QWORD PTR t$[rsp], rax

; 293  : 	memset(t, 0, sizeof(thread_t));

	mov	r8d, 1104				; 00000450H
	xor	edx, edx
	mov	rcx, QWORD PTR t$[rsp]
	call	memset

; 294  : 	t->frame.ss = SEGVAL(GDT_ENTRY_USER_DATA,3); 

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax], 35			; 00000023H

; 295  : 	t->frame.rsp = (uint64_t)stack;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR stack$[rsp]
	mov	QWORD PTR [rax+8], rcx

; 296  : 	t->frame.rflags = 0x286;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+16], 646			; 00000286H

; 297  : 	t->frame.cs = SEGVAL (GDT_ENTRY_USER_CODE,3);

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+24], 43			; 0000002bH

; 298  : 	t->frame.rip = (uint64_t)entry;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR entry$[rsp]
	mov	QWORD PTR [rax+32], rcx

; 299  : 	t->frame.rax = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+40], 0

; 300  : 	t->frame.rbx = 10;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+48], 10

; 301  : 	t->frame.rcx = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+56], 0

; 302  : 	t->frame.rdx = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+64], 0

; 303  : 	t->frame.rsi = 10;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+72], 10

; 304  : 	t->frame.rdi = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+80], 0

; 305  : 	t->frame.rbp = (uint64_t)t->frame.rsp;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR [rcx+8]
	mov	QWORD PTR [rax+88], rcx

; 306  : 	t->frame.r8 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+96], 0

; 307  : 	t->frame.r9 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+104], 0

; 308  : 	t->frame.r10 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+112], 0

; 309  : 	t->frame.r11 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+120], 0

; 310  : 	t->frame.r12 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+128], 0

; 311  : 	t->frame.r13 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+136], 0

; 312  : 	t->frame.r14 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+144], 0

; 313  : 	t->frame.r15 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+152], 0

; 314  : 
; 315  : 	/** Kernel stack is important for syscall or interruption in the system **/
; 316  : 	t->frame.kern_esp = (uint64_t)allocate_kstack_child((uint64_t*)p2v((size_t)parent->frame.cr3));

	mov	rax, QWORD PTR parent$[rsp]
	mov	rcx, QWORD PTR [rax+192]
	call	p2v
	mov	rcx, rax
	call	?allocate_kstack_child@@YA_KPEA_K@Z	; allocate_kstack_child
	mov	rcx, QWORD PTR t$[rsp]
	mov	QWORD PTR [rcx+200], rax

; 317  : 	t->user_stack = stack;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR stack$[rsp]
	mov	QWORD PTR [rax+208], rcx

; 318  : 	t->frame.ds = 0x23;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+160], 35			; 00000023H

; 319  : 	t->frame.es = 0x23;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+168], 35			; 00000023H

; 320  : 	t->frame.fs = 0x23;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+176], 35			; 00000023H

; 321  : 	t->frame.gs = 0x23;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+184], 35			; 00000023H

; 322  : 	t->frame.cr3 = parent->frame.cr3;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR parent$[rsp]
	mov	rcx, QWORD PTR [rcx+192]
	mov	QWORD PTR [rax+192], rcx

; 323  : 	memset(t->name, 0, 8);

	mov	rax, QWORD PTR t$[rsp]
	add	rax, 229				; 000000e5H
	mov	r8d, 8
	xor	edx, edx
	mov	rcx, rax
	call	memset

; 324  : 	strncpy(t->name, name, 8);

	mov	rax, QWORD PTR t$[rsp]
	add	rax, 229				; 000000e5H
	mov	r8d, 8
	mov	rdx, QWORD PTR name$[rsp]
	mov	rcx, rax
	call	strncpy

; 325  : 	t->id = task_id++;

	mov	rax, QWORD PTR t$[rsp]
	movzx	ecx, WORD PTR ?task_id@@3GA		; task_id
	mov	WORD PTR [rax+238], cx
	movzx	eax, WORD PTR ?task_id@@3GA		; task_id
	inc	ax
	mov	WORD PTR ?task_id@@3GA, ax		; task_id

; 326  : 	t->quanta = 0;

	xor	eax, eax
	mov	rcx, QWORD PTR t$[rsp]
	mov	WORD PTR [rcx+242], ax

; 327  : 	t->msg_box = NULL; //parent->msg_box;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+248], 0

; 328  : 	t->fx_state = (uint8_t*)malloc(512);

	mov	ecx, 512				; 00000200H
	call	malloc
	mov	rcx, QWORD PTR t$[rsp]
	mov	QWORD PTR [rcx+216], rax

; 329  : 	memset(t->fx_state, 0, 512);

	mov	r8d, 512				; 00000200H
	xor	edx, edx
	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR [rax+216]
	call	memset

; 330  : 	t->mxcsr = 0x1f80;

	mov	rax, QWORD PTR t$[rsp]
	mov	DWORD PTR [rax+224], 8064		; 00001f80H

; 331  : 	t->_is_user = 1;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+228], 1

; 332  : 	t->priviledge = THREAD_LEVEL_USER;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+240], 2

; 333  : 	t->state = THREAD_STATE_READY;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+237], 1

; 334  : 	t->priority = 1;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+256], 1

; 335  : 	t->fd_current = 3;

	mov	rax, QWORD PTR t$[rsp]
	mov	DWORD PTR [rax+744], 3

; 336  : 	thread_insert (t);

	mov	rcx, QWORD PTR t$[rsp]
	call	?thread_insert@@YAXPEAU_thread_@@@Z	; thread_insert

; 337  : 	return t;

	mov	rax, QWORD PTR t$[rsp]

; 338  : }

	add	rsp, 56					; 00000038H
	ret	0
create_child_thread ENDP
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
t$ = 32
entry$ = 64
stack$ = 72
cr3$ = 80
name$ = 88
priority$ = 96
create_kthread PROC

; 170  : {

$LN3:
	mov	QWORD PTR [rsp+32], r9
	mov	QWORD PTR [rsp+24], r8
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	sub	rsp, 56					; 00000038H

; 171  : 	thread_t *t = (thread_t*)malloc(sizeof(thread_t));//pmmngr_alloc();

	mov	ecx, 1104				; 00000450H
	call	malloc
	mov	QWORD PTR t$[rsp], rax

; 172  : 	memset(t, 0, sizeof(thread_t));

	mov	r8d, 1104				; 00000450H
	xor	edx, edx
	mov	rcx, QWORD PTR t$[rsp]
	call	memset

; 173  : 	t->frame.ss = 0x10;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax], 16

; 174  : 	t->frame.rsp = (uint64_t)stack;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR stack$[rsp]
	mov	QWORD PTR [rax+8], rcx

; 175  : 	t->frame.rflags = 0x202;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+16], 514			; 00000202H

; 176  : 	t->frame.cs = 0x08;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+24], 8

; 177  : 	t->frame.rip = (uint64_t)entry;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR entry$[rsp]
	mov	QWORD PTR [rax+32], rcx

; 178  : 	t->frame.rax = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+40], 0

; 179  : 	t->frame.rbx = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+48], 0

; 180  : 	t->frame.rcx = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+56], 0

; 181  : 	t->frame.rdx = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+64], 0

; 182  : 	t->frame.rsi = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+72], 0

; 183  : 	t->frame.rdi = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+80], 0

; 184  : 	t->frame.rbp = (uint64_t)t->frame.rsp;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR [rcx+8]
	mov	QWORD PTR [rax+88], rcx

; 185  : 	t->frame.r8 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+96], 0

; 186  : 	t->frame.r9 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+104], 0

; 187  : 	t->frame.r10 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+112], 0

; 188  : 	t->frame.r11 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+120], 0

; 189  : 	t->frame.r12 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+128], 0

; 190  : 	t->frame.r13 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+136], 0

; 191  : 	t->frame.r14 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+144], 0

; 192  : 	t->frame.r15 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+152], 0

; 193  : 	t->frame.ds = 0x10;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+160], 16

; 194  : 	t->frame.es = 0x10;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+168], 16

; 195  : 	t->frame.fs = 0x10;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+176], 16

; 196  : 	t->frame.gs = 0x10;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+184], 16

; 197  : 	t->frame.kern_esp = stack;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR stack$[rsp]
	mov	QWORD PTR [rax+200], rcx

; 198  : 	t->user_stack = stack;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR stack$[rsp]
	mov	QWORD PTR [rax+208], rcx

; 199  : 	t->_is_user = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+228], 0

; 200  : 	t->frame.cr3 = cr3;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR cr3$[rsp]
	mov	QWORD PTR [rax+192], rcx

; 201  : 	memset(t->name, 0, 8);

	mov	rax, QWORD PTR t$[rsp]
	add	rax, 229				; 000000e5H
	mov	r8d, 8
	xor	edx, edx
	mov	rcx, rax
	call	memset

; 202  : 	strncpy(t->name, name, 8);

	mov	rax, QWORD PTR t$[rsp]
	add	rax, 229				; 000000e5H
	mov	r8d, 8
	mov	rdx, QWORD PTR name$[rsp]
	mov	rcx, rax
	call	strncpy

; 203  : 	t->id = task_id++;

	mov	rax, QWORD PTR t$[rsp]
	movzx	ecx, WORD PTR ?task_id@@3GA		; task_id
	mov	WORD PTR [rax+238], cx
	movzx	eax, WORD PTR ?task_id@@3GA		; task_id
	inc	ax
	mov	WORD PTR ?task_id@@3GA, ax		; task_id

; 204  : 	t->quanta = 0;

	xor	eax, eax
	mov	rcx, QWORD PTR t$[rsp]
	mov	WORD PTR [rcx+242], ax

; 205  : 	t->priviledge = THREAD_LEVEL_KERNEL;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+240], 1

; 206  : 	t->state = THREAD_STATE_READY;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+237], 1

; 207  : 	//t->priority = priority;
; 208  : 	t->fd_current = 3;

	mov	rax, QWORD PTR t$[rsp]
	mov	DWORD PTR [rax+744], 3

; 209  : 	t->fx_state = (uint8_t*)malloc(512);

	mov	ecx, 512				; 00000200H
	call	malloc
	mov	rcx, QWORD PTR t$[rsp]
	mov	QWORD PTR [rcx+216], rax

; 210  : 	memset(t->fx_state, 0, 512);

	mov	r8d, 512				; 00000200H
	xor	edx, edx
	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR [rax+216]
	call	memset

; 211  : 	t->mxcsr = 0x1f80;

	mov	rax, QWORD PTR t$[rsp]
	mov	DWORD PTR [rax+224], 8064		; 00001f80H

; 212  : 	/* Signal not supported for kernel threads */
; 213  : 	t->signal_queue = NULL;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+1064], 0

; 214  : 	t->pending_signal_count = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	DWORD PTR [rax+1072], 0

; 215  : 	thread_insert(t);

	mov	rcx, QWORD PTR t$[rsp]
	call	?thread_insert@@YAXPEAU_thread_@@@Z	; thread_insert

; 216  : 	return t;

	mov	rax, QWORD PTR t$[rsp]

; 217  : }

	add	rsp, 56					; 00000038H
	ret	0
create_kthread ENDP
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
t$ = 32
tv178 = 40
entry$ = 64
stack$ = 72
cr3$ = 80
name$ = 88
priority$ = 96
?create_user_thread@@YAPEAU_thread_@@P6AXPEAX@Z_K2QEADE@Z PROC ; create_user_thread

; 228  : {

$LN3:
	mov	QWORD PTR [rsp+32], r9
	mov	QWORD PTR [rsp+24], r8
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	sub	rsp, 56					; 00000038H

; 229  : 	thread_t *t = (thread_t*)malloc(sizeof(thread_t));//pmmngr_alloc();

	mov	ecx, 1104				; 00000450H
	call	malloc
	mov	QWORD PTR t$[rsp], rax

; 230  : 	memset (t, 0, sizeof(thread_t));

	mov	r8d, 1104				; 00000450H
	xor	edx, edx
	mov	rcx, QWORD PTR t$[rsp]
	call	memset

; 231  : 	t->frame.ss = SEGVAL(GDT_ENTRY_USER_DATA,3); 

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax], 35			; 00000023H

; 232  : 	t->frame.rsp = (uint64_t)stack;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR stack$[rsp]
	mov	QWORD PTR [rax+8], rcx

; 233  : 	t->frame.rflags = 0x286;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+16], 646			; 00000286H

; 234  : 	t->frame.cs = SEGVAL (GDT_ENTRY_USER_CODE,3);

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+24], 43			; 0000002bH

; 235  : 	t->frame.rip = (uint64_t)entry;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR entry$[rsp]
	mov	QWORD PTR [rax+32], rcx

; 236  : 	t->frame.rax = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+40], 0

; 237  : 	t->frame.rbx = 10;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+48], 10

; 238  : 	t->frame.rcx = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+56], 0

; 239  : 	t->frame.rdx = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+64], 0

; 240  : 	t->frame.rsi = 10;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+72], 10

; 241  : 	t->frame.rdi = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+80], 0

; 242  : 	t->frame.rbp = (uint64_t)t->frame.rsp;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR [rcx+8]
	mov	QWORD PTR [rax+88], rcx

; 243  : 	t->frame.r8 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+96], 0

; 244  : 	t->frame.r9 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+104], 0

; 245  : 	t->frame.r10 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+112], 0

; 246  : 	t->frame.r11 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+120], 0

; 247  : 	t->frame.r12 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+128], 0

; 248  : 	t->frame.r13 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+136], 0

; 249  : 	t->frame.r14 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+144], 0

; 250  : 	t->frame.r15 = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+152], 0

; 251  : 
; 252  : 	/** Kernel stack is important for syscall or interruption in the system **/
; 253  : 	t->frame.kern_esp = (uint64_t)allocate_kstack((uint64_t*)cr3);

	mov	rcx, QWORD PTR cr3$[rsp]
	call	?allocate_kstack@@YA_KPEA_K@Z		; allocate_kstack
	mov	rcx, QWORD PTR t$[rsp]
	mov	QWORD PTR [rcx+200], rax

; 254  : 	t->user_stack = stack;

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR stack$[rsp]
	mov	QWORD PTR [rax+208], rcx

; 255  : 	t->frame.ds = 0x23;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+160], 35			; 00000023H

; 256  : 	t->frame.es = 0x23;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+168], 35			; 00000023H

; 257  : 	t->frame.fs = 0x23;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+176], 35			; 00000023H

; 258  : 	t->frame.gs = 0x23;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+184], 35			; 00000023H

; 259  : 	t->frame.cr3 = v2p(cr3);

	mov	rcx, QWORD PTR cr3$[rsp]
	call	v2p
	mov	rcx, QWORD PTR t$[rsp]
	mov	QWORD PTR [rcx+192], rax

; 260  : 	memset(t->name, 0, 8);

	mov	rax, QWORD PTR t$[rsp]
	add	rax, 229				; 000000e5H
	mov	r8d, 8
	xor	edx, edx
	mov	rcx, rax
	call	memset

; 261  : 	strncpy (t->name, name, 8);

	mov	rax, QWORD PTR t$[rsp]
	add	rax, 229				; 000000e5H
	mov	r8d, 8
	mov	rdx, QWORD PTR name$[rsp]
	mov	rcx, rax
	call	strncpy

; 262  : 	t->id = task_id++;

	mov	rax, QWORD PTR t$[rsp]
	movzx	ecx, WORD PTR ?task_id@@3GA		; task_id
	mov	WORD PTR [rax+238], cx
	movzx	eax, WORD PTR ?task_id@@3GA		; task_id
	inc	ax
	mov	WORD PTR ?task_id@@3GA, ax		; task_id

; 263  : 	t->quanta = 0;

	xor	eax, eax
	mov	rcx, QWORD PTR t$[rsp]
	mov	WORD PTR [rcx+242], ax

; 264  : 	t->msg_box = (uint64_t*)p2v((size_t)AuPmmngrAlloc());

	call	AuPmmngrAlloc
	mov	rcx, rax
	call	p2v
	mov	rcx, QWORD PTR t$[rsp]
	mov	QWORD PTR [rcx+248], rax

; 265  : 	/** Map the thread's msg box to a virtual address, from where the process will receive system messages **/
; 266  : 	AuMapPageEx((uint64_t*)p2v(t->frame.cr3),v2p((size_t)t->msg_box),(uint64_t)0x400000, PAGING_USER);

	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR [rax+248]
	call	v2p
	mov	QWORD PTR tv178[rsp], rax
	mov	rcx, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR [rcx+192]
	call	p2v
	mov	r9b, 4
	mov	r8d, 4194304				; 00400000H
	mov	rcx, QWORD PTR tv178[rsp]
	mov	rdx, rcx
	mov	rcx, rax
	call	?AuMapPageEx@@YA_NPEA_K_K1E@Z		; AuMapPageEx

; 267  : 
; 268  : 	t->fx_state = (uint8_t*)malloc(512);

	mov	ecx, 512				; 00000200H
	call	malloc
	mov	rcx, QWORD PTR t$[rsp]
	mov	QWORD PTR [rcx+216], rax

; 269  : 	memset(t->fx_state, 0, 512);

	mov	r8d, 512				; 00000200H
	xor	edx, edx
	mov	rax, QWORD PTR t$[rsp]
	mov	rcx, QWORD PTR [rax+216]
	call	memset

; 270  : 	t->mxcsr = 0x1f80;

	mov	rax, QWORD PTR t$[rsp]
	mov	DWORD PTR [rax+224], 8064		; 00001f80H

; 271  : 	t->_is_user = 1;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+228], 1

; 272  : 	t->priviledge = THREAD_LEVEL_USER;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+240], 2

; 273  : 	t->state = THREAD_STATE_READY;

	mov	rax, QWORD PTR t$[rsp]
	mov	BYTE PTR [rax+237], 1

; 274  : 	t->priority = priority;

	mov	rax, QWORD PTR t$[rsp]
	movzx	ecx, BYTE PTR priority$[rsp]
	mov	BYTE PTR [rax+256], cl

; 275  : 	t->fd_current = 3;

	mov	rax, QWORD PTR t$[rsp]
	mov	DWORD PTR [rax+744], 3

; 276  : 
; 277  : 	t->signal_queue = NULL;

	mov	rax, QWORD PTR t$[rsp]
	mov	QWORD PTR [rax+1064], 0

; 278  : 	t->pending_signal_count = 0;

	mov	rax, QWORD PTR t$[rsp]
	mov	DWORD PTR [rax+1072], 0

; 279  : 	thread_insert (t);

	mov	rcx, QWORD PTR t$[rsp]
	call	?thread_insert@@YAXPEAU_thread_@@@Z	; thread_insert

; 280  : 	return t;

	mov	rax, QWORD PTR t$[rsp]

; 281  : }

	add	rsp, 56					; 00000038H
	ret	0
?create_user_thread@@YAPEAU_thread_@@P6AXPEAX@Z_K2QEADE@Z ENDP ; create_user_thread
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
?AuSchedulerStart@@YAXXZ PROC				; AuSchedulerStart

; 492  : void AuSchedulerStart () {

$LN3:
	sub	rsp, 40					; 00000028H

; 493  : #ifdef USE_APIC
; 494  : 	setvect(0x40, scheduler_isr);

	lea	rdx, OFFSET FLAT:?scheduler_isr@@YAX_KPEAX@Z ; scheduler_isr
	mov	ecx, 64					; 00000040H
	call	setvect

; 495  : #endif
; 496  : #ifdef USE_PIC
; 497  : 	AuInterruptSet(0,scheduler_isr,0, false);
; 498  : #endif
; 499  : 	execute_idle(current_thread,get_kernel_tss());

	call	get_kernel_tss
	mov	rdx, rax
	mov	rcx, QWORD PTR current_thread
	call	execute_idle

; 500  : }

	add	rsp, 40					; 00000028H
	ret	0
?AuSchedulerStart@@YAXXZ ENDP				; AuSchedulerStart
_TEXT	ENDS
; Function compile flags: /Odtpy
; File e:\xeneva project\xeneva\aurora\aurora\arch\x86_64\thread.cpp
_TEXT	SEGMENT
tv67 = 48
idle_$ = 56
?AuInitializeScheduler@@YAXXZ PROC			; AuInitializeScheduler

; 350  : void AuInitializeScheduler () {

$LN3:
	sub	rsp, 72					; 00000048H

; 351  : 	//blocked_list = initialize_list();
; 352  : 	scheduler_enable = true;

	mov	BYTE PTR ?scheduler_enable@@3_NA, 1	; scheduler_enable

; 353  : 	scheduler_initialized = true;

	mov	BYTE PTR ?scheduler_initialized@@3_NA, 1 ; scheduler_initialized

; 354  : 	task_id = 0;

	xor	eax, eax
	mov	WORD PTR ?task_id@@3GA, ax		; task_id

; 355  : 	/** Here create the first thread, the idle thread which never gets
; 356  : 	    blocked nor get destroyed untill the system turns off **/
; 357  : 	thread_t *idle_ = create_kthread (idle_thread,(uint64_t)p2v((uint64_t)AuPmmngrAlloc() + 4096),x64_read_cr3(),"Idle",1);

	call	x64_read_cr3
	mov	QWORD PTR tv67[rsp], rax
	call	AuPmmngrAlloc
	add	rax, 4096				; 00001000H
	mov	rcx, rax
	call	p2v
	mov	BYTE PTR [rsp+32], 1
	lea	r9, OFFSET FLAT:$SG3803
	mov	rcx, QWORD PTR tv67[rsp]
	mov	r8, rcx
	mov	rdx, rax
	lea	rcx, OFFSET FLAT:?idle_thread@@YAXXZ	; idle_thread
	call	create_kthread
	mov	QWORD PTR idle_$[rsp], rax

; 358  : 	current_thread = idle_;

	mov	rax, QWORD PTR idle_$[rsp]
	mov	QWORD PTR current_thread, rax

; 359  : 	AuPCPUSetCurrentThread(current_thread);

	mov	rcx, QWORD PTR current_thread
	call	?AuPCPUSetCurrentThread@@YAXPEAU_thread_@@@Z ; AuPCPUSetCurrentThread

; 360  : }

	add	rsp, 72					; 00000048H
	ret	0
?AuInitializeScheduler@@YAXXZ ENDP			; AuInitializeScheduler
_TEXT	ENDS
END
